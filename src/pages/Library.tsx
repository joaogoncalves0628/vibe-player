...
const Library = () => {
  ...
  return (
    ...
    >
      <div className="px-4 sm:px-6 pt-8 sm:pt-12 max-w-screen-xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8"
        >
          <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl gradient-primary flex items-center justify-center shadow-xl">
            <Heart className="w-5 h-5 sm:w-7 sm:h-7 text-primary-foreground fill-current" />
          </div>
          <div>
            <h1 className="text-2xl sm:text-3xl font-display font-bold text-foreground">Músicas Curtidas</h1>
            <p className="text-sm sm:text-base text-muted-foreground">{likedSongs.length} música{likedSongs.length !== 1 ? "s" : ""}</p>
          </div>
        </motion.div>
